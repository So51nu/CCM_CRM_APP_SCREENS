    ...
    return SingleChildScrollView(
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          KeyValueRow('Lead Owner', _text(lead.raw['assigned_to_name'] ?? lead.raw['owner'] ?? CrmScope.of(context).userName)),
          KeyValueRow('Created On', _text(lead.raw['created_at'] ?? lead.raw['created_on'] ?? '-')), 
        ...
        GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          KeyValueRow('Email', lead.email.isEmpty ? '-' : lead.email, icon: Icons.email_outlined),
          KeyValueRow('Company', lead.company, icon: Icons.business_rounded),
          KeyValueRow('Designation', _text(lead.raw['designation'] ?? lead.raw['job_title'] ?? '-'), icon: Icons.badge_outlined),
          KeyValueRow('Address', _text(lead.raw['address'] ?? lead.city), icon: Icons.location_on_outlined),
        ])),
        const SizedBox(height: 12),
        GlassCard(
